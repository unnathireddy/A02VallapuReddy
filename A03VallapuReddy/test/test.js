...
QUnit.test("Here's a test that should always pass", function (assert) {
    assert.ok(1 <= "3", "1❤ - the first agrument is 'truthy', so we pass!");
});

QUnit.test('Testing calculateTotalSeats function with several sets of inputs', function (assert) {
    assert.equal(App.calculateTotalSeats(5, 5), 25, 'Tested whether two numbers are positive');
    assert.equal(App.calculateTotalSeats(-5, -5), 1, 'Tested whether a number is positive or negative');
    assert.equal(App.calculateTotalSeats(500, 500), 10000, 'with missing arguments(throw an error)');
        //throws( block                                    [, expected ] [, message ] ) 
    assert.throws(function () { App.calculateTotalSeats(null); }, 'Tested whether with two negative numbers');
        //throws( block                                    [, expected ] [, message ] ) 
    assert.throws(function () { App.calculateTotalSeats("Christine","Christine"); }, "Tested if two numbers are zero");
});